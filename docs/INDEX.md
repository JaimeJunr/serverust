...
- [**guides/auth.md**](guides/auth.md) — Guia de autenticação com `serverust-auth`: verificação de JWT de IdP externo com chave estática (`JwtAuth`), `AuthLayer` + extractors `Auth`/`MaybeAuth`, claims próprias via `AuthzFacts`, códigos de erro do 401 e o que ainda não está implementado.
