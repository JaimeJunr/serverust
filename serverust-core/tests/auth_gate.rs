...
// ---------------------------------------------------------------------------
// Escape hatch de migração (`allow_unannotated`)
// ---------------------------------------------------------------------------

#[tokio::test]
async fn allow_unannotated_abre_rota_nao_marcada() {
    let router = App::new()
        .without_docs()
        .interceptor(AutenticacaoFalsa {
            identidade_valida: false,
...
/// Como o resto do desenho, o escape hatch decide em runtime sobre um
/// marcador — então chamá-lo depois de registrar as rotas dá o mesmo
/// resultado. Se dependesse da montagem, esta ordem o tornaria inócuo em
/// silêncio.
#[tokio::test]
async fn allow_unannotated_independe_da_ordem_do_builder() {
    let router = App::new()
        .without_docs()
        .route(privado)
        .interceptor(AutenticacaoFalsa {
...
/// O escape hatch afrouxa o **default**, não a autorização explícita: quem
/// pediu identidade na assinatura continua sendo atendido pelo extractor.
#[tokio::test]
async fn allow_unannotated_nao_muda_o_caminho_autenticado() {
    let router = App::new()
        .without_docs()
        .interceptor(AutenticacaoFalsa {
            identidade_valida: true,
...
// ---------------------------------------------------------------------------
// Inventário de rotas públicas (mitigação 2 da decisão 5)
// ---------------------------------------------------------------------------

#[test]
fn public_routes_lista_so_o_que_foi_marcado() {
    let app = App::new().without_docs().route(privado).route(RotaPublica);

    assert_eq!(
        app.public_routes(),
        &[("GET", "/health")],
        "o inventário é a declaração de qual é a superfície anônima — rota \
         protegida entrando aqui a torna falsa"
    );
}

#[test]
fn sem_rota_publica_o_inventario_fica_vazio() {
    let app = App::new().without_docs().route(privado);

    assert!(app.public_routes().is_empty());
}
